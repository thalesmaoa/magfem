...
export type CanvasTab = { kind: 'draw' } | { kind: 'view'; id: string } | { kind: 'chart'; plot: string } | { kind: 'bh'; material: string } | { kind: 'circuits'; physics: string } | { kind: 'table'; id: string };

export const tabKey = (t: CanvasTab) =>
  t.kind === 'draw' ? 'draw' : t.kind === 'view' ? `view:${t.id}` : t.kind === 'chart' ? `chart:${t.plot}` : t.kind === 'bh' ? `bh:${t.material}` : t.kind === 'table' ? `table:${t.id}` : `circuits:${t.physics}`;
